...
return view.extend({
	load: function() {
		var tasks = [
			...
			fs.trimmed('/morse/scripts/morse_default.sh')
		];
	...
	handleMorseReset: function(ev) {
		if (!confirm(_('Do you really want to erase Morse settings?')))
			return;

		ui.showModal(_('Resetting...'), [
			E('p', { 'class': 'spinning' }, _('The Morse configs are being reverted...'))
		]);

		/* Currently the sysupgrade rpc call will not return, hence no promise handling */
		fs.exec_direct("/morse/scripts/morse_default.sh");
	...
	render: function(rpc_replies) {
		...
		o = s.option(form.SectionValue, 'actions', form.NamedSection, 'actions', 'actions', _('Reset Defaults'), _('To reset the firmware to its initial state, click "Perform reset" (only possible with squashfs images).'));
		ss = o.subsection;
		...
		} else if(has_morse_reset) {
			o = ss.option(form.Button, 'reset', _('Reset Morse'), _('Only resets Morse configuration files and interfaces.'));
			o.inputstyle = 'negative important';
			o.inputtitle = _('Perform reset');
			o.onclick = this.handleMorseReset;
		}

		o = s.option(form.SectionValue, 'actions', form.NamedSection, 'actions', 'actions', 'Restore', _('To restore configuration files, you can upload a previously generated backup archive here. '));
		ss = o.subsection;
