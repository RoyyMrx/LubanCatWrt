...
function format_channel(country, band, chanNum, freqMHz) {
	if (band == 's1g') {
		// We want to report the bandwidth of the channel for s1g, so we 'need' the channelMap.
		// It would obviously be nicer if iwinfo freqlist reported this info...
		const channel = channelMap[country][chanNum];
		return '%d (%f MHz, %f MHz bandwidth)'.format(channel.s1g_chan, channel.centre_freq_mhz, channel.bw);
	} else {
...
// Allow in place updating of the list.
// This allows us to do things like update the channel list when
// we change the country.
var CBIModifiableListValue = form.ListValue.extend({
	clear: function() {
...
var CBIWifiFrequencyValue = form.Value.extend({
	...
	load: function(section_id) {
		return Promise.all([
			...
			const hasS1g = uci.get('wireless', section_id, 'band') == 's1g' || uci.get('wireless', section_id, 'hwmode') == '11ah';
			const country = uci.get('wireless', section_id, 'country') || Object.keys(channelMap)[0];
			if (hasS1g) {
				for (const c of Object.values(channelMap[country])) {
					this.channels['s1g'].push(c.s1g_chan, format_channel(country, 's1g', c.s1g_chan, c.centre_freq_mhz), true);
				}
			} else {
				for (var i = 0; i < data[1].length; i++) {
					...
					this.channels[band].push(
						data[1][i].channel,
						format_channel(country, band, data[1][i].channel, data[1][i].mhz),
						!data[1][i].restricted
					);
				}
			}
	...
	// Currently only used for s1g, since it's more important to react
	// to country updates immediately.
	updateChannels: function(band, country, channels) {
		this.channels[band] = [];
		for (const chanInfo of channels) {
			this.channels[band].push(chanInfo.channel, format_channel(country, band, chanInfo.channel, chanInfo.mhz), !chanInfo.restricted);
		}
		this.toggleWifiChannel(this.elem);
	},
	...
	addChannelEventListener: function(type, listener) {
		this.elem.querySelector('.channel').addEventListener(type, listener);
	},
	...
	renderWidget: function(section_id, option_index, cfgvalue) {
		var elem = this.elem = E('div');

		let channelEl, widthEl;
		...
		for (const optionEl of [channelEl, widthEl]) {
		...
		s.addModalOptions = function(s) {
			return network.getWifiNetwork(s.section).then(function(radioNet) {
				...
				if (hwtype == 'mac80211') {
					...
					const countryValue = ss.taboption('general', CBIWifiCountryValue, 'country', _('Country Code'));
					countryValue.wifiNetwork = radioNet;

					const freqValue = ss.taboption('general', CBIWifiFrequencyValue, '_freq', '<br />' + _('Operating frequency'));
					freqValue.ucisection = s.section;

					// We wrap s1g_capab in custom stuff to present a sane interface to SHORT-GI
					// (i.e. so users can't select invalid combos).
					// The easy way would be to just use a single DynamicList and validate, or change
					// the uci configuration so it echos the dual structure here, but let's try to be fancy.
					// Also, if there are ever other things added to s1g_capab...
					const s1g_capab_sgi_o = o = ss.taboption('advanced', form.ListValue, 's1g_capab_sgi', _('Short Guard Interval'));
					o.value('[SHORT-GI-ALL]', _('All'));
					o.value('[SHORT-GI-NONE]', _('None'))
					o.value('[SHORT-GI-CUSTOM]', _('Custom'));  // 'Fake' value used to select custom SHORT-GI in next option.
					o.default = '[SHORT-GI-ALL]';
					o.load = (section_id) => {
						const s1g_capabs = uci.get('wireless', section_id, 's1g_capab') ?? [];
						if (s1g_capabs.includes('[SHORT-GI-ALL]')) {
							return '[SHORT-GI-ALL]';
						} else if (s1g_capabs.includes('[SHORT-GI-NONE]')) {
							return '[SHORT-GI-NONE]';
						} else if (s1g_capabs.some(f => f.startsWith('[SHORT-GI-'))) {
							return '[SHORT-GI-CUSTOM]';
						}
					};
					o.write = (section_id, formvalue) => {
						if (formvalue && formvalue !== '[SHORT-GI-CUSTOM]') {
							let s1g_capabs = uci.get('wireless', section_id, 's1g_capab') ?? [];
							s1g_capabs = s1g_capabs.filter(f => !f.startsWith('[SHORT-GI'));
							s1g_capabs.push(formvalue);
							uci.set('wireless', section_id, 's1g_capab', s1g_capabs);
						}
					};

					const s1g_capab_sgi_custom_o = o = ss.taboption('advanced', form.MultiValue, 's1g_capab_sgi_custom', _('Short Guard Interval Frequencies'));
					o.depends('s1g_capab_sgi', '[SHORT-GI-CUSTOM]')
					o.value('[SHORT-GI-1]', _('1 MHz'));
					o.value('[SHORT-GI-2]', _('2 MHz'));
					o.value('[SHORT-GI-4]', _('4 MHz'));
					o.value('[SHORT-GI-8]', _('8 MHz'));
					o.load = (section_id) => {
						const s1g_capabs = uci.get('wireless', section_id, 's1g_capab') ?? [];
						return s1g_capabs.filter(f => /^\[SHORT-GI-\d+]$/.test(f));
					};
					o.write = (section_id, formvalue) => {
						if (s1g_capab_sgi_o.formvalue(section_id) === '[SHORT-GI-CUSTOM]') {
							let s1g_capabs = uci.get('wireless', section_id, 's1g_capab') ?? [];
							s1g_capabs = s1g_capabs.filter(f => !f.startsWith('[SHORT-GI'));
							s1g_capabs.push(...formvalue);
							uci.set('wireless', section_id, 's1g_capab', s1g_capabs);
					...
					o = ss.taboption('advanced', form.Value, 'forced_listen_interval', _('Forced listen interval (AP and STA)'));
					...
					const primChanIndex = ss.taboption('advanced', CBIModifiableListValue, 's1g_prim_1mhz_chan_index', _('Primary 1MHz channel index'))
					...
					primChanIndex.load = function(section_id) {
						...
						if (country && channel) {
							this.updateOptions(country, channel);
						}
						return form.ListValue.prototype.load.apply(this, [section_id]);
					}

					// Only 1MHz and 2MHz supported.
					const primChanWidth = ss.taboption('advanced', CBIModifiableListValue, 's1g_prim_chwidth', _('Primary channel width'),
					...
					primChanWidth.load = function(section_id) {
						const country = uci.get('wireless', section_id, 'country');
						const channel = uci.get('wireless', section_id, 'channel');
						const configPrimChanIndex = uci.get('wireless', section_id, 's1g_prim_1mhz_chan_index');
						if (country && channel && configPrimChanIndex) {
							this.updateOptions(country, channel, configPrimChanIndex);
						}
						return form.ListValue.prototype.load.apply(this, [section_id]);
					}

					primChanIndex.updateOptions = (country, channel) => {
						primChanIndex.clear();
						const bw = Number(channelMap[country][channel]?.bw ?? 1);
					...
					primChanWidth.updateOptions = (country, channel, newChanIndex) => {
						const bw = Number(channelMap[country][channel]?.bw ?? 1);
						primChanWidth.clear();
						primChanWidth.value(1, '1MHz');

						if (bw >= 4 && (!newChanIndex || newChanIndex < bw - 1)) {
							// i.e. if our channel is wide enough and the index
							// is not right at the top of the bandwidth.
					...
					primChanIndex.onchange = (ev, section_id, value) => {
						primChanWidth.updateOptions(countryValue.formvalue(section_id), freqValue.formvalue(section_id)[2], value);
						primChanWidth.renderUpdate(section_id);
					}

					freqValue.onchange = (ev, section_id, value) => {
						const [htmode, band, channel] = value;
						primChanIndex.updateOptions(countryValue.formvalue(section_id), channel);
						primChanIndex.renderUpdate(section_id);
						primChanIndex.onchange(ev, section_id, primChanIndex.formvalue(section_id));
					};

					countryValue.onchange = (ev, section_id, country) => {
						freqValue.updateChannels('s1g', country, Object.values(channelMap[country]).map(c => ({
							channel: c.s1g_chan, mhz: c.centre_freq_mhz, restricted: false,
						})));
						freqValue.onchange(ev, section_id, freqValue.formvalue(section_id));
					};
				...
				if (hwtype == 'mac80211') {
					...
					o = ss.taboption('advanced', form.Flag, 'cac', _('Centralised authentication control'), _('Dynamically adjusts the portion of stations which can send AuthReq messages'));
					o.enabled = '1';
					o.disabled = '0';
					o.default = o.disabled;
				}
				...
				if(hwtype == 'morse') {
					...
					o = ss.taboption('powersave', form.Value, 'wake_interval', _('Wake interval'));
					o.depends('twt', '1');
					o.placeholder = '1000000';

					o = ss.taboption('powersave', form.Value, 'min_wake_duration', _('Minimum wake duration'));
					o.depends('twt', '1');
					o.placeholder = '65982';

					o = ss.taboption('powersave', form.ListValue, 'setup_command', _('Setup command'));
					o.value('0', _('Request'));
					o.value('1', _('Suggest'));
					o.default = '0';
					o.depends('twt', '1');
					...
					os.handleAdd = function(ev, name) {
						...
						let ifaceRaws = this.map.data.get('wireless', wifi_iface_section_name, 'raws').filter(r => r !== name);
						ifaceRaws.push(name);
					...
					os.handleRemove = function(section_id, ev) {
						...
						let ifaceRaws = this.map.data.get('wireless', wifi_iface_section_name, 'raws').filter(r => r !== section_id);
