...
function getItfMac(itf, ifconfigOutput)
{
    const regexItf = new RegExp(itf);
    const regexMac = new RegExp(/(HWaddr\s)((\w{2})\W){6}/);
    var itfStr = ifconfigOutput.substring(regexItf.exec(ifconfigOutput)['index']);
	
    return regexMac.exec(itfStr)[0].replace("HWaddr ","");
}


function getItfIp(itf, ifconfigOutput)
{
    var ip_address;
    const regexItf = new RegExp(itf);
    const regexip = new RegExp(/(inet addr:)([0-9]{1,3}\W){3}[0-9]{0,3}/);
    var itfStr = ifconfigOutput.substring(regexItf.exec(ifconfigOutput)['index']);
    if (regexip.test(itfStr)) {
        var ip_address = regexip.exec(itfStr)[0].replace("inet addr:","");
    } else {
        var ip_address = 'N/A'
    }
    return ip_address;
